...
<hr>
  ...
  <br>
  @foreach($skillme as $skill)
  <div align="left">
  <div class="completion-bar">
   <div class="progress-bar" style="width:{{ $skill->skill_percentage}};background-color:{{ $skill->skill_color}}"> <i class="{{ $skill->skill_icon}}"></i> {{ $skill->skill_name }}</div>
  <div class="progress_value">{{ $skill->skill_percentage }}</div>  
  </div>
  </div>
  @endforeach 
  <!--<div align="left">
 ...
 @foreach($skilltransfer as $skill)
  <div align="left">
  <div class="completion-bar">
  <div class="progress-bar" style="width:{{ $skill->skill_percentage}};background-color:{{ $skill->skill_color}}"> <i class="{{ $skill->skill_icon}}"></i> {{ $skill->skill_name }}</div>
  <div class="progress_value">{{ $skill->skill_percentage }}</div>     
...
<div style='display: grid; margin-top: 20px; text-align: center'>
	...
	<table cellspacing="0">
		<tr>
		@foreach($skillLanguage as $skill)
		<td>
		<div class="counter" data-cp-percentage="{{ $skill->skill_percentage }}" data-cp-color="{{ $skill->skill_color}}">
		</div>
		</td>
		<!--<td>
